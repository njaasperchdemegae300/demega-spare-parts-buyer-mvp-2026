...
const reportPath = path.join(ROOT, "reports", "version11b-hot-buyer-dashboard-smoke-test-report.md");
  ...
  try {
    ...
    const createAction = await request("/api/action-queue/create", {
      ...
      body: JSON.stringify({
        ...
        priority: "urgent",
        note: "Dashboard test: call hot buyer manually.",
        assignedTo: "master_admin"
      })
    });

    const page = await request("/hot-buyers");
    const aliasPage = await request("/hot-buyer-command-center");
    ...
    const pageOk =
      page.status === 200 &&
      page.text.includes("Demega Hot Buyer Command Center") &&
      page.text.includes("Ranked Hot Buyers") &&
      page.text.includes("Hot Score") &&
      page.text.includes("Recommended Manual Actions") &&
      page.text.includes("Read-only serious-buyer ranking") &&
      page.text.includes("READ ONLY") &&
      page.text.includes("No auto-send") &&
      page.text.includes("No auto-quote") &&
      page.text.includes("No auto-pipeline") &&
      page.text.includes("hotBuyerRows");

    const aliasOk =
      aliasPage.status === 200 &&
      aliasPage.text.includes("Demega Hot Buyer Command Center");
    ...
    const report = `# Version 11B Hot Buyer Dashboard Smoke Test Report
...
- ${createActionOk ? "PASS" : "FAIL"}: hot buyer manual action created for dashboard display
- ${pageOk ? "PASS" : "FAIL"}: GET /hot-buyers returns Hot Buyer Command Center dashboard
- ${aliasOk ? "PASS" : "FAIL"}: GET /hot-buyer-command-center alias works
- ${listOk ? "PASS" : "FAIL"}: GET /api/hot-buyers returns ranked hot buyer data
- ${summaryOk ? "PASS" : "FAIL"}: GET /api/hot-buyers/summary returns safe dashboard metrics
- ${readOnlyOk ? "PASS" : "FAIL"}: Hot Buyer Command Center dashboard remains read-only
