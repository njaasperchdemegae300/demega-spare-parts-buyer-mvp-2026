...
const reportPath = path.join(ROOT, "reports", "version35b-controlled-buyer-gate-manual-quote-draft-dashboard-smoke-test-report.md");
...
function safeSlotPayload(index) {
  return {
    leadReference: `controlled-dashboard-manual-quote-draft-test-lead-${index}`,
    partNeeded: index % 2 === 0 ? "Toyota Corolla alternator" : "Toyota Corolla kick starter",
    vehicleDetail: "Toyota Corolla 2005",
    buyerLocation: "Lagos",
    buyerIntentProof: "Buyer initiated WhatsApp click-to-chat inbound request during manual quote draft dashboard test.",
...
function safeReviewPayload(slotNumber, decision) {
  return {
    ...
    reviewNote: "Manual lead review before manual quote draft dashboard. No buyer contact. No quote sent.",
...
function safeStockCheckPayload(slotNumber, stockDecision) {
  return {
    ...
    stockNote: "Manual stock check before manual quote draft dashboard. No buyer contact. No quote sent. No inventory mutation.",
...
function safeCompatibilityPayload(slotNumber, compatibilityDecision) {
  return {
    ...
    compatibilityNote: "Manual compatibility before manual quote draft dashboard. No buyer contact. No quote sent. No price sent.",
...
function safeFinalEligibilityPayload(slotNumber, eligibilityDecision) {
  return {
    slotNumber,
    eligibilityDecision,
    eligibilityNote: "Final quote eligibility before manual quote draft dashboard. No buyer contact. No quote sent.",
...
function safeManualQuoteDraftPayload(slotNumber, price) {
  return {
    ...
    quoteNote: "Manual quote draft dashboard test only. Do not send automatically.",
    checkedBy: "master_admin",
...
async function main() {
  ...
  try {
    ...
    if (!health) {
      const startupReport = `# Version 35B Controlled Buyer-Gate Manual Quote Draft Dashboard Smoke Test Report
...
`;
    ...
    const page = await request("/controlled-buyer-gate-manual-quote-draft");
    const aliasPage = await request("/controlled-buyer-gate-manual-quote-drafts");
    ...
    const pageOk =
      page.status === 200 &&
      page.text.includes("Demega Controlled Buyer-Gate Manual Quote Draft Dashboard") &&
      page.text.includes("Manual Quote Draft Safety Rule") &&
      page.text.includes("Manual Quote Draft Scope") &&
      page.text.includes("Controlled Buyer-Gate Manual Quote Draft Records") &&
      page.text.includes("Manual quote draft gate only") &&
      page.text.includes("Manual quote draft record only") &&
      page.text.includes("Controlled manual quote draft only") &&
      page.text.includes("Final quote eligibility required first") &&
      page.text.includes("ELIGIBLE_FOR_MANUAL_QUOTE_DRAFT required first") &&
      page.text.includes("Price allowed only inside internal draft") &&
      page.text.includes("No buyer contact from draft gate") &&
      page.text.includes("No quote sent at draft gate") &&
      page.text.includes("No price sent to buyer") &&
      ...
      page.text.includes("Manual review before sending required next") &&
      page.text.includes("Manual send confirmation required next") &&
      page.text.includes("MANUAL_QUOTE_DRAFT_PREPARED") &&
      page.text.includes("INTERNAL_DRAFT_ONLY") &&
      page.text.includes("PRICE_IN_DRAFT_ONLY") &&
      page.text.includes("QUOTE_NOT_SENT_TO_BUYER") &&
      page.text.includes("PRICE_NOT_SENT_TO_BUYER") &&
      page.text.includes("Internal Draft Price") &&
      page.text.includes("draftRows");

    const aliasOk =
      aliasPage.status === 200 &&
      aliasPage.text.includes("Demega Controlled Buyer-Gate Manual Quote Draft Dashboard");

    const listOk =
      list.status === 200 &&
      list.body &&
      Array.isArray(list.body.manualQuoteDrafts) &&
      list.body.manualQuoteDrafts.length === 2 &&
    ...
    const summaryOk =
      ...
      summary.body.summary.totalManualQuoteDrafts === 2 &&
      summary.body.summary.preparedManualQuoteDraftCount === 2 &&
      summary.body.summary.eligibleManualQuoteDraftCount === 2 &&
    ...
    const readOnlyOk =
      ...
      !page.text.includes("method: 'POST'") &&
      !page.text.includes('fetch("/api/controlled-buyer-gate-manual-quote-draft/create"');
    ...
    const report = `# Version 35B Controlled Buyer-Gate Manual Quote Draft Dashboard Smoke Test Report
...
- ${draftsOk ? "PASS" : "FAIL"}: manual quote draft records exist for dashboard display
- ${pageOk ? "PASS" : "FAIL"}: GET /controlled-buyer-gate-manual-quote-draft returns safe dashboard
- ${aliasOk ? "PASS" : "FAIL"}: GET /controlled-buyer-gate-manual-quote-drafts alias works
- ${listOk ? "PASS" : "FAIL"}: manual quote draft list API returns dashboard data safely
- ${summaryOk ? "PASS" : "FAIL"}: manual quote draft summary API confirms safe dashboard metrics
- ${readOnlyOk ? "PASS" : "FAIL"}: Manual Quote Draft dashboard remains read-only
